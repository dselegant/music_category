...
def create_fft(g, n):
    ...
    sad = r"./trainset/" + g + "." + str(n).zfill(5) + ".fft"
...
for g in genre_list:
    for n in range(100):
        rad = r"./trainset/" + g + "." + str(n).zfill(5) + ".fft" + ".npy"
        # 加载文件
...
# prepare test data-------------------
# sample_rate, test = wavfile.read(r"./heibao-wudizirong-remix.wav")
# sample_rate, test = wavfile.read(r"./Ring05.wav")
sample_rate, test = wavfile.read(r"./country.00000.au.wav")
# sample_rate, test = wavfile.read(r"./xialian.wav")
# sample_rate, test = wavfile.read(r"./love_story.wav")
print(sample_rate, test)

testdata_fft_features = abs(fft(test)[:1000])
# print(testdata_fft_features)
print(testdata_fft_features.shape)
# model.predict(testdata_fft_features) 预测为一个数组，array([类别])
type_index = model.predict(testdata_fft_features.reshape(1, -1))[0]
print(model.score(X, Y))
print(type_index)
print('音乐类型', genre_list[type_index])
